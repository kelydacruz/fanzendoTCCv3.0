...
async function renderPagina(res, edicao = '', dados = {}, erro = '', status = 200) {
    return res.status(status).render('admin/turmas', {
        title: 'Turmas técnicas', turmas: await listarTurmas(), cursos: await listarCursos(), edicao, dados, erro,
    });
}
...
export default class TurmaController {
    constructor() {
        ...
        this.add = async (req, res, next) => {
            const dados = { nome: req.body.nome, ano: req.body.ano, curso: req.body.curso };
            try {
                await cadastrarTurma(dados);
                return res.redirect('/admin/turmas?mensagem=' + encodeURIComponent('Cadastro realizado com sucesso.'));
            } catch (erro) {
        ...
        this.openEdt = async (req, res, next) => {
            try {
                const item = await buscarTurma(req.params.id);
        ...
        this.edt = async (req, res, next) => {
            const dados = { nome: req.body.nome, ano: req.body.ano, curso: req.body.curso };
            try {
                const item = await atualizarTurma(req.params.id, dados);
                if (!item) return res.status(404).render('404', { title: 'Cadastro não encontrado' });
                return res.redirect('/admin/turmas?mensagem=' + encodeURIComponent('Cadastro atualizado com sucesso.'));
            } catch (erro) {
        ...
        this.status = async (req, res, next) => {
            try {
                ...
                const item = await atualizarTurma(req.params.id, { ativo: req.body.ativo === 'true' });
                if (!item) return res.status(404).render('404', { title: 'Cadastro não encontrado' });
                return res.redirect('/admin/turmas?mensagem=' + encodeURIComponent('Situação atualizada.'));
            } catch (erro) {
        ...
        this.confirmDelete = async (req, res, next) => {
            try {
                ...
                return res.render('admin/confirmar-remocao', {
                    title: 'Excluir turma', nome: item.nome,
                    explicacao: 'A exclusão será permitida somente se não houver vínculos. Se houver, use Desativar para preservar o histórico.',
                    destino: `/admin/turmas/${req.params.id}/excluir`, voltar: '/admin/turmas', busca: '',
                    token: prepararConfirmacao(req, 'turma', req.params.id),
                });
            } catch (erro) { return next(erro); }
        };
        ...
        this.del = async (req, res, next) => {
            try {
                if (!consumirConfirmacao(req, 'turma', req.params.id)) return res.status(403).render('erro', {
                    title: 'Confirmação necessária', mensagemErro: 'Abra novamente a confirmação de exclusão.',
                });
                const item = await excluirTurma(req.params.id);
                if (!item) return res.status(404).render('404', { title: 'Cadastro não encontrado' });
                return res.redirect('/admin/turmas?mensagem=' + encodeURIComponent('Cadastro excluído com sucesso. Os cadastros existentes foram preservados.'));
            } catch (erro) {
                if (erroEsperado(erro)) return renderPagina(res, '', {}, erro.message, erro.status || 400).catch(next);
                return next(erro);
            }
